...
**Note:** this is the "per-architecture" repository for the `s390x` builds of [the `arangodb` official image](https://hub.docker.com/_/arangodb) -- for more information, see ["Architectures other than amd64?" in the official images documentation](https://github.com/docker-library/official-images#architectures-other-than-amd64) and ["An image's source changed in Git, now what?" in the official images FAQ](https://github.com/docker-library/faq#an-images-source-changed-in-git-now-what).
...
```console
unix> docker run -e ARANGO_RANDOM_ROOT_PASSWORD=1 s390x/arangodb --help
```
...
```console
unix> docker run -it s390x/arangodb arangosh --server.database myDB ...
```
