...
**Note:** this is the "per-architecture" repository for the `riscv64` builds of [the `friendica` official image](https://hub.docker.com/_/friendica) -- for more information, see ["Architectures other than amd64?" in the official images documentation](https://github.com/docker-library/official-images#architectures-other-than-amd64) and ["An image's source changed in Git, now what?" in the official images FAQ](https://github.com/docker-library/faq#an-images-source-changed-in-git-now-what).
...
[![riscv64/friendica build status badge](https://img.shields.io/jenkins/s/https/doi-janky.infosiftr.net/job/multiarch/job/riscv64/job/friendica.svg?label=riscv64/friendica%20%20build%20job)](https://doi-janky.infosiftr.net/job/multiarch/job/riscv64/job/friendica/)
...
```console
$ docker run -d -p 8080:80 --network some-network riscv64/friendica
```
...
```console
$ docker run -d riscv64/friendica:fpm
```
...
You have to pull the latest image from the hub (`docker pull riscv64/friendica`). The stable branch gets checked at every startup and will get updated if no installation was found or a new image is used.
