...
**Note:** this is the "per-architecture" repository for the `riscv64` builds of [the `hitch` official image](https://hub.docker.com/_/hitch) -- for more information, see ["Architectures other than amd64?" in the official images documentation](https://github.com/docker-library/official-images#architectures-other-than-amd64) and ["An image's source changed in Git, now what?" in the official images FAQ](https://github.com/docker-library/faq#an-images-source-changed-in-git-now-what).
...
[![riscv64/hitch build status badge](https://img.shields.io/jenkins/s/https/doi-janky.infosiftr.net/job/multiarch/job/riscv64/job/hitch.svg?label=riscv64/hitch%20%20build%20job)](https://doi-janky.infosiftr.net/job/multiarch/job/riscv64/job/hitch/)
...
```console
$ docker run --name=hitch -p 443:443 riscv64/hitch
```
...
```console
$ docker run -v /path/to/your/config/file:/etc/hitch/hitch.conf:ro riscv64/hitch
```
...
```console
$ docker run riscv64/hitch "--backend=[backend.example.com]:8443"
```
...
```console
$ docker run -v /path/to/your/certificate:/etc/hitch/certs/default:ro riscv64/hitch
```
...
```console
$ docker run riscv64/hitch "--backend=[backend.example.com]:8443:/path/to/cert.pem"
```
