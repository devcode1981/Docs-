...
**Note:** this is the "per-architecture" repository for the `riscv64` builds of [the `mediawiki` official image](https://hub.docker.com/_/mediawiki) -- for more information, see ["Architectures other than amd64?" in the official images documentation](https://github.com/docker-library/official-images#architectures-other-than-amd64) and ["An image's source changed in Git, now what?" in the official images FAQ](https://github.com/docker-library/faq#an-images-source-changed-in-git-now-what).
...
[![riscv64/mediawiki build status badge](https://img.shields.io/jenkins/s/https/doi-janky.infosiftr.net/job/multiarch/job/riscv64/job/mediawiki.svg?label=riscv64/mediawiki%20%20build%20job)](https://doi-janky.infosiftr.net/job/multiarch/job/riscv64/job/mediawiki/)
...
```console
$ docker run --name some-mediawiki -d riscv64/mediawiki
```

If you'd like to be able to access the instance from the host without the container's IP, standard port mappings can be used:

```console
$ docker run --name some-mediawiki -p 8080:80 -d riscv64/mediawiki
```
...
```console
$ docker run --name some-mediawiki --link some-mysql:mysql -d riscv64/mediawiki
```
...
```console
$ docker run --rm riscv64/mediawiki tar -cC /var/www/html/sites . | tar -xC /path/on/host/sites
```
