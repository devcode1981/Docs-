...
**Note:** this is the "per-architecture" repository for the `mips64le` builds of [the `mongo` official image](https://hub.docker.com/_/mongo) -- for more information, see ["Architectures other than amd64?" in the official images documentation](https://github.com/docker-library/official-images#architectures-other-than-amd64) and ["An image's source changed in Git, now what?" in the official images FAQ](https://github.com/docker-library/faq#an-images-source-changed-in-git-now-what).
...
[![mips64le/mongo build status badge](https://img.shields.io/jenkins/s/https/doi-janky.infosiftr.net/job/multiarch/job/mips64le/job/mongo.svg?label=mips64le/mongo%20%20build%20job)](https://doi-janky.infosiftr.net/job/multiarch/job/mips64le/job/mongo/)
...
```console
$ docker run --name some-mongo -d mips64le/mongo:tag
```
...
```console
$ docker run -it --network some-network --rm mips64le/mongo mongosh --host some-mongo test
```
...
The `docker exec` command allows you to run commands inside a Docker container. The following command line will give you a bash shell inside your `mips64le/mongo` container:
...
```console
$ docker run --name some-mongo -d mips64le/mongo --serviceExecutor adaptive
```
...
```console
$ docker run -it --rm mips64le/mongo --help
```
...
```console
$ docker run --name some-mongo -d mips64le/mongo --wiredTigerCacheSizeGB 1.5
```
...
For a more complicated configuration setup, you can still use the MongoDB configuration file. `mongod` does not read a configuration file by default, so the `--config` option with the path to the configuration file needs to be specified. Create a custom configuration file and put it in the container by either creating a custom Dockerfile `FROM mips64le/mongo` or mounting it from the host machine to the container. See the MongoDB manual for a full list of [configuration file](https://docs.mongodb.com/manual/reference/configuration-options/) options.
...
```console
$ docker run --name some-mongo -v /my/custom:/etc/mongo -d mips64le/mongo --config /etc/mongo/mongod.conf
```
...
$ docker run -it --rm --network some-network mips64le/mongo \
...
```console
$ docker run --name some-mongo -e MONGO_INITDB_ROOT_PASSWORD_FILE=/run/secrets/mongo-root -d mips64le/mongo
```
...
2.	Start your `mongo` container like this:
	...
	```console
	$ docker run --name some-mongo -v /my/own/datadir:/data/db -d mips64le/mongo
	```
