...
[![riscv64/nats-streaming build status badge](https://img.shields.io/jenkins/s/https/doi-janky.infosiftr.net/job/multiarch/job/riscv64/job/nats-streaming.svg?label=riscv64/nats-streaming%20%20build%20job)](https://doi-janky.infosiftr.net/job/multiarch/job/riscv64/job/nats-streaming/)
...
```bash
$ docker run -p 4223:4223 -p 8223:8223 riscv64/nats-streaming nats-streaming-server -p 4223 -m 8223
```

If you need to specify the entrypoint:

```bash
$ docker run --entrypoint c:/nats-streaming-server/nats-streaming-server -p 4222:4222 -p 8222:8222 riscv64/nats-streaming
```
...
```bash
$ docker run -p 4223:4223 -p 8223:8223 riscv64/nats-streaming -p 4223 -m 8223
```

If you need to specify the entrypoint:

```bash
$ docker run --entrypoint /nats-streaming-server -p 4222:4222 -p 8222:8222 riscv64/nats-streaming
```
...
#
#   docker run -p 4444:4444 riscv64/nats-streaming -p 4444
#
# Or, if you want to publish the port 4444 as a different port, for example 5555:
#
#   docker run -p 5555:4444 riscv64/nats-streaming -p 4444
#
# Check "docker run" for more information.

$ docker run -d -p 4222:4222 -p 8222:8222 riscv64/nats-streaming
```
...
```bash
$ docker run -d -p 4222:4222 -p 8222:8222 riscv64/nats-streaming -store file -dir datastore
...
```bash
$ docker run -d --link nats-main riscv64/nats-streaming -store file -dir datastore -ns nats://nats-main:4222
