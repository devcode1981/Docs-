...
**Note:** this is the "per-architecture" repository for the `riscv64` builds of [the `odoo` official image](https://hub.docker.com/_/odoo) -- for more information, see ["Architectures other than amd64?" in the official images documentation](https://github.com/docker-library/official-images#architectures-other-than-amd64) and ["An image's source changed in Git, now what?" in the official images FAQ](https://github.com/docker-library/faq#an-images-source-changed-in-git-now-what).
...
[![riscv64/odoo build status badge](https://img.shields.io/jenkins/s/https/doi-janky.infosiftr.net/job/multiarch/job/riscv64/job/odoo.svg?label=riscv64/odoo%20%20build%20job)](https://doi-janky.infosiftr.net/job/multiarch/job/riscv64/job/odoo/)
...
```console
$ docker run -p 8069:8069 --name odoo --link db:db -t riscv64/odoo
```
...
```console
$ docker run -v /path/to/config:/etc/odoo -p 8069:8069 --name odoo --link db:db -t riscv64/odoo
```
...
```console
$ docker run -p 8069:8069 --name odoo --link db:db -t riscv64/odoo -- --db-filter=odoo_db_.*
```
...
```console
$ docker run -v /path/to/addons:/mnt/extra-addons -p 8069:8069 --name odoo --link db:db -t riscv64/odoo
```

## Run multiple Odoo instances

```console
$ docker run -p 8070:8069 --name odoo2 --link db:db -t riscv64/odoo
$ docker run -p 8071:8069 --name odoo3 --link db:db -t riscv64/odoo
```
...
```console
$ docker run --volumes-from old-odoo -p 8070:8069 --name new-odoo --link db:db -t riscv64/odoo
```
