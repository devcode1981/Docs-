...
**Note:** this is the "per-architecture" repository for the `riscv64` builds of [the `orientdb` official image](https://hub.docker.com/_/orientdb) -- for more information, see ["Architectures other than amd64?" in the official images documentation](https://github.com/docker-library/official-images#architectures-other-than-amd64) and ["An image's source changed in Git, now what?" in the official images FAQ](https://github.com/docker-library/faq#an-images-source-changed-in-git-now-what).
...
[![riscv64/orientdb build status badge](https://img.shields.io/jenkins/s/https/doi-janky.infosiftr.net/job/multiarch/job/riscv64/job/orientdb.svg?label=riscv64/orientdb%20%20build%20job)](https://doi-janky.infosiftr.net/job/multiarch/job/riscv64/job/orientdb/)
...
```console
$ docker run -d --name orientdb -p 2424:2424 -p 2480:2480 -e ORIENTDB_ROOT_PASSWORD=rootpwd riscv64/orientdb
```
...
```console
$ docker run --rm -it riscv64/orientdb /orientdb/bin/console.sh
```

or even the etl

```console
$ docker run  --rm -it -v <config_path>:/orientdb/config riscv64/orientdb /orientdb/bin/oetl.sh ../config/oetl-config.json
```
...
$ docker run -d --name orientdb -p 2424:2424 -p 2480:2480 \
    ...
    riscv64/orientdb /orientdb/bin/server.sh  -Ddistributed=true
```
...
$ docker run -d --name orientdb -p 2424:2424 -p 2480:2480 \
    -e ORIENTDB_ROOT_PASSWORD=rootpwd \
    riscv64/orientdb /orientdb/bin/server.sh -Xmx8g
```
