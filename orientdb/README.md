...
**Note:** this is the "per-architecture" repository for the `mips64le` builds of [the `orientdb` official image](https://hub.docker.com/_/orientdb) -- for more information, see ["Architectures other than amd64?" in the official images documentation](https://github.com/docker-library/official-images#architectures-other-than-amd64) and ["An image's source changed in Git, now what?" in the official images FAQ](https://github.com/docker-library/faq#an-images-source-changed-in-git-now-what).
...
[![mips64le/orientdb build status badge](https://img.shields.io/jenkins/s/https/doi-janky.infosiftr.net/job/multiarch/job/mips64le/job/orientdb.svg?label=mips64le/orientdb%20%20build%20job)](https://doi-janky.infosiftr.net/job/multiarch/job/mips64le/job/orientdb/)
...
```console
$ docker run -d --name orientdb -p 2424:2424 -p 2480:2480 -e ORIENTDB_ROOT_PASSWORD=rootpwd mips64le/orientdb
```
...
```console
$ docker run --rm -it mips64le/orientdb /orientdb/bin/console.sh
```

or even the etl

```console
$ docker run  --rm -it -v <config_path>:/orientdb/config mips64le/orientdb /orientdb/bin/oetl.sh ../config/oetl-config.json
```
...
$ docker run -d --name orientdb -p 2424:2424 -p 2480:2480 \
    ...
    mips64le/orientdb /orientdb/bin/server.sh  -Ddistributed=true
```
...
$ docker run -d --name orientdb -p 2424:2424 -p 2480:2480 \
    -e ORIENTDB_ROOT_PASSWORD=rootpwd \
    mips64le/orientdb /orientdb/bin/server.sh -Xmx8g
```
