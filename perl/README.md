...
```console
$ docker run -it --rm --name my-running-script -v "$PWD":/usr/src/myapp -w /usr/src/myapp winamd64/perl:5.34 perl your-daemon-or-script.pl
```
...
```console
$ docker run -it --name sleeping_beauty --rm winamd64/perl:5.34 perl -E 'sleep 300'
```
...
```console
$ docker run -it --name quick_nap --rm winamd64/perl:5.34 perl -E '$SIG{TERM} = sub { $sig++; say "recv TERM" }; sleep 300; say "waking up" if $sig'
```
