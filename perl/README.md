...
**Note:** this is the "per-architecture" repository for the `arm32v6` builds of [the `perl` official image](https://hub.docker.com/_/perl) -- for more information, see ["Architectures other than amd64?" in the official images documentation](https://github.com/docker-library/official-images#architectures-other-than-amd64) and ["An image's source changed in Git, now what?" in the official images FAQ](https://github.com/docker-library/faq#an-images-source-changed-in-git-now-what).
...
```console
$ docker run -it --rm --name my-running-script -v "$PWD":/usr/src/myapp -w /usr/src/myapp arm32v6/perl:5.34 perl your-daemon-or-script.pl
```
...
```console
$ docker run -it --name sleeping_beauty --rm arm32v6/perl:5.34 perl -E 'sleep 300'
```
...
```console
$ docker run -it --name quick_nap --rm arm32v6/perl:5.34 perl -E '$SIG{TERM} = sub { $sig++; say "recv TERM" }; sleep 300; say "waking up" if $sig'
```
