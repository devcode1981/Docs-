...
**Note:** this is the "per-architecture" repository for the `riscv64` builds of [the `plone` official image](https://hub.docker.com/_/plone) -- for more information, see ["Architectures other than amd64?" in the official images documentation](https://github.com/docker-library/official-images#architectures-other-than-amd64) and ["An image's source changed in Git, now what?" in the official images FAQ](https://github.com/docker-library/faq#an-images-source-changed-in-git-now-what).
...
[![riscv64/plone build status badge](https://img.shields.io/jenkins/s/https/doi-janky.infosiftr.net/job/multiarch/job/riscv64/job/plone.svg?label=riscv64/plone%20%20build%20job)](https://doi-janky.infosiftr.net/job/multiarch/job/riscv64/job/plone/)
...
```console
$ docker run -p 8080:8080 riscv64/plone
```
...
```console
$ docker run --name=zeo riscv64/plone zeo
```

Start 2 Plone clients (also in the background)

```console
$ docker run --link=zeo -e ZEO_ADDRESS=zeo:8080 -p 8081:8080 riscv64/plone
$ docker run --link=zeo -e ZEO_ADDRESS=zeo:8080 -p 8082:8080 riscv64/plone
```
...
```console
$ docker run -p 8080:8080 riscv64/plone fg
```
...
```console
$ docker run -p 8080:8080 -e PLONE_ADDONS="eea.facetednavigation Products.PloneFormGen" riscv64/plone
```
...
```console
$ docker run -p 8080:8080 -e SITE="mysite" -e ADDONS="eea.facetednavigation collective.easyform" riscv64/plone
```
...
```console
$ docker run -p 8080:8080 -e SITE=plone riscv64/plone
