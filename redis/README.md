...
**Note:** this is the "per-architecture" repository for the `riscv64` builds of [the `redis` official image](https://hub.docker.com/_/redis) -- for more information, see ["Architectures other than amd64?" in the official images documentation](https://github.com/docker-library/official-images#architectures-other-than-amd64) and ["An image's source changed in Git, now what?" in the official images FAQ](https://github.com/docker-library/faq#an-images-source-changed-in-git-now-what).
...
[![riscv64/redis build status badge](https://img.shields.io/jenkins/s/https/doi-janky.infosiftr.net/job/multiarch/job/riscv64/job/redis.svg?label=riscv64/redis%20%20build%20job)](https://doi-janky.infosiftr.net/job/multiarch/job/riscv64/job/redis/)
...
```console
$ docker run --name some-redis -d riscv64/redis redis-server --save 60 1 --loglevel warning
```
...
```console
$ docker run -it --network some-network --rm riscv64/redis redis-cli -h some-redis
```
...
```console
$ docker run -v /myredis/conf:/usr/local/etc/redis --name myredis riscv64/redis redis-server /usr/local/etc/redis/redis.conf
```
