...
```console
$ docker run --rm -v "$PWD":/usr/src/app -w /usr/src/app winamd64/ruby:3.0 bundle install
```
...
```console
$ docker run -it --rm --name my-running-script -v "$PWD":/usr/src/myapp -w /usr/src/myapp winamd64/ruby:3.0 ruby your-daemon-or-script.rb
```
