...
**Note:** this is the "per-architecture" repository for the `riscv64` builds of [the `spiped` official image](https://hub.docker.com/_/spiped) -- for more information, see ["Architectures other than amd64?" in the official images documentation](https://github.com/docker-library/official-images#architectures-other-than-amd64) and ["An image's source changed in Git, now what?" in the official images FAQ](https://github.com/docker-library/faq#an-images-source-changed-in-git-now-what).
...
[![riscv64/spiped build status badge](https://img.shields.io/jenkins/s/https/doi-janky.infosiftr.net/job/multiarch/job/riscv64/job/spiped.svg?label=riscv64/spiped%20%20build%20job)](https://doi-janky.infosiftr.net/job/multiarch/job/riscv64/job/spiped/)
...
```console
$ docker run -it --rm riscv64/spiped
...
```console
$ docker run -d -v /path/to/keyfile:/spiped/key:ro -p 8025:8025 --init riscv64/spiped -d -s '[0.0.0.0]:8025' -t '[127.0.0.1]:25'
```
...
```console
$ docker run -d -v /path/to/keyfile:/spiped/key:ro -p 9200:9200 --link elasticsearch:elasticsearch --init riscv64/spiped -d -s '[0.0.0.0]:9200' -t 'elasticsearch:9200'
```

If you don’t need any to bind to a privileged port you can pass `--user spiped` to make *spiped* run as an unprivileged user:

```console
$ docker run -d -v /path/to/keyfile:/spiped/key:ro --user spiped -p 9200:9200 --link elasticsearch:elasticsearch --init riscv64/spiped -d -s '[0.0.0.0]:9200' -t 'elasticsearch:9200'
```
...
```console
$ docker run -it --rm -v /path/to/keyfile:/spiped/key riscv64/spiped spiped-generate-key.sh
```
