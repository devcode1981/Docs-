...
**Note:** this is the "per-architecture" repository for the `mips64le` builds of [the `storm` official image](https://hub.docker.com/_/storm) -- for more information, see ["Architectures other than amd64?" in the official images documentation](https://github.com/docker-library/official-images#architectures-other-than-amd64) and ["An image's source changed in Git, now what?" in the official images FAQ](https://github.com/docker-library/faq#an-images-source-changed-in-git-now-what).
...
[![mips64le/storm build status badge](https://img.shields.io/jenkins/s/https/doi-janky.infosiftr.net/job/multiarch/job/mips64le/job/storm.svg?label=mips64le/storm%20%20build%20job)](https://doi-janky.infosiftr.net/job/multiarch/job/mips64le/job/storm/)
...
```console
$ docker run -it -v $(pwd)/topology.jar:/topology.jar mips64le/storm storm jar /topology.jar org.apache.storm.starter.ExclamationTopology
```
...
2.	The Nimbus daemon has to be connected with the Zookeeper. It's also a "fail fast" system.
	...
	```console
	$ docker run -d --restart always --name some-nimbus --link some-zookeeper:zookeeper mips64le/storm storm nimbus
	```

3.	Finally start a single Supervisor node. It will talk to the Nimbus and Zookeeper.

	```console
	$ docker run -d --restart always --name supervisor --link some-zookeeper:zookeeper --link some-nimbus:nimbus mips64le/storm storm supervisor
	```

4.	Now you can submit a topology to our cluster.

	```console
	$ docker run --link some-nimbus:nimbus -it --rm -v $(pwd)/topology.jar:/topology.jar mips64le/storm storm jar /topology.jar org.apache.storm.starter.WordCountTopology topology
	```

5.	Optionally, you can start the Storm UI.

	```console
	$ docker run -d -p 8080:8080 --restart always --name ui --link some-nimbus:nimbus mips64le/storm storm ui
	```
...
1.	Using command line arguments.
	...
	```console
	$ docker run -d --restart always --name nimbus mips64le/storm storm nimbus -c storm.zookeeper.servers='["zookeeper"]'
	```

2.	Assuming you have `storm.yaml` in the current directory you can mount it as a volume.

	```console
	$ docker run -it -v $(pwd)/storm.yaml:/conf/storm.yaml mips64le/storm storm nimbus
	```
...
```console
$ docker run -it -v /logs -v /data mips64le/storm storm nimbus
```
