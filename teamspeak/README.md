...
**Note:** this is the "per-architecture" repository for the `mips64le` builds of [the `teamspeak` official image](https://hub.docker.com/_/teamspeak) -- for more information, see ["Architectures other than amd64?" in the official images documentation](https://github.com/docker-library/official-images#architectures-other-than-amd64) and ["An image's source changed in Git, now what?" in the official images FAQ](https://github.com/docker-library/faq#an-images-source-changed-in-git-now-what).
...
[![mips64le/teamspeak build status badge](https://img.shields.io/jenkins/s/https/doi-janky.infosiftr.net/job/multiarch/job/mips64le/job/teamspeak.svg?label=mips64le/teamspeak%20%20build%20job)](https://doi-janky.infosiftr.net/job/multiarch/job/mips64le/job/teamspeak/)
...
```console
$ docker run -e TS3SERVER_LICENSE=view mips64le/teamspeak
```

To start a TeamSpeak server, accept the license agreement, and map the ports to the host:

```console
$ docker run -p 9987:9987/udp -p 10011:10011 -p 30033:30033 -e TS3SERVER_LICENSE=accept mips64le/teamspeak
```
...
```console
$ docker run --name some-teamspeak -v /location/to/licensekey.dat:/var/ts3server/licensekey.dat mips64le/teamspeak:tag
```
...
```console
$ docker run --name some-teamspeak -v /my/own/datadir:/var/ts3server/ -d mips64le/teamspeak:tag
```
